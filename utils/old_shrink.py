...
def generate_summary(file_name, top_n, output):
    ...
    tokens = 0
    selected_sentences = [sentence for score, sentence in ranked_sentences[:current_selected]] #! just a precaution
    while tokens < 3800  and current_selected < len(ranked_sentences):
        ...
        current_selected += 20 #keep selecting more until at limit
...
for filename in os.listdir(folder_path):
        ...
        if os.path.isfile(file_path):
            print(file_path)            
            processed_content = generate_summary(file_path, 80, output) #! might need to fix np
